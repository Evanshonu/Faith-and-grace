  ...
  const handleSubmit = async e => {
    ...
    try {
      const { error: stripeError, paymentIntent } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          // Stripe redirects HERE if 3D Secure is required — OrderConfirmation handles saving
      ...
      // No redirect was needed — payment succeeded immediately
...
const Checkout = () => {
  ...
  const handleInfoSubmit = async info => {
    ...
    // ── KEY: persist cart snapshot + customer info to localStorage NOW
    // This is the safety net for 3D Secure redirects — if Stripe redirects
    // the user away for bank auth, OrderConfirmation reads this to save the order
    localStorage.setItem('customerInfo', JSON.stringify(info));
    localStorage.setItem('pendingOrderCart', JSON.stringify(
      cart.map(i => ({ name: i.name, qty: i.quantity, price: i.price }))
    ));
    ...
    try {
      ...
      setInitError(err.message || 'Unable to connect to payment server. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSuccess = async paymentIntentId => {
    ...
    // Clear cart — updates badge everywhere instantly
    clearCart();
