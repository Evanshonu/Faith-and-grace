...
const CustomerForm = ({ onSubmit }) => {
  ...
  const inputSty = { background: 'rgba(255,255,255,0.07)', border: '2px solid rgba(255,255,255,0.1)', fontFamily: 'inherit' };
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input value={form.name} onChange={e => set('name', e.target.value)}
            placeholder="Your name" className={inputCls} style={inputSty} />
        </div>
        <div>
          <label className="text-xs font-black tracking-widest uppercase text-stone-500 block mb-2">Phone *</label>
          <input value={form.phone} onChange={e => set('phone', e.target.value)}
            placeholder="Your phone number" className={inputCls} style={inputSty} />
        </div>
      </div>
      ...
      <div>
        ...
        <input value={form.email} onChange={e => set('email', e.target.value)}
          placeholder="your@email.com" className={inputCls} style={inputSty} />
      </div>
      ...
      {form.method === 'delivery' && (
        <div>
          ...
          <input value={form.address} onChange={e => set('address', e.target.value)}
            placeholder="Full delivery address" className={inputCls} style={inputSty} />
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          >
            <div className="rounded-3xl p-6 sm:p-8"
              ...
              {step === 'info' && (
                <CustomerForm onSubmit={handleInfoSubmit} />
              )}
              ...
              {/* Success */}
              {step === 'payment' && !loading && success && <SuccessState paymentId={successId} />}
